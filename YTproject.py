...
#Connecting YouTube
api_key = 'AIzaSyBf9jTHssK-6dn_NqOX_Di1f1l9-_rRtOw' 
...
##Connecting MongoDB 
Nivetha=pymongo.MongoClient('mongodb+srv://nivetha:Nivi1234@cluster0.mzw5alx.mongodb.net/?retryWrites=true&w=majority')
db = Nivetha['YT_data'] #create database
coll = db['youtube'] #Collection to store details
...
def migrate_to_sql(import_from_mongodb):
    ...
    #to migrate channel details to channel tabel
...
with col1:
    #Getting Channel ID as an input in Streamlit
...
with col2:
    ...
    if search_button:
        try:
            ...
            st.warning("Channel ID entered in invalid")
          
col4,col5,col6 = st.columns([1,3,1])

with col6:# to export data to Mongo DB to store in a Data Lake
    st.text("")
    st.text("")
    ...
    if migrate_button:
        try:
            ...
            try:
                ...
                if selected_option == df.channel_name.iloc[0]:
                    st.warning("Duplicate channel_id, Data already exists")
                else:
                    pass
            except IndexError: 
...
questions = [
    ...
    "5.What is the total number of likes for each video, and what are their corresponding video names?",
...
#Queries and questions are equated correspopnding to the index value
...
# Dropdown is introduced to select the query question
selected_question = st.selectbox("Select a question:", questions)

# Executing the corresponding query based on the selected question
...
if selected_question:
    ...
    cursor.execute(query)
    results = cursor.fetchall()
    # The output is convetred DataFrame
    df = pd.DataFrame(results, columns=[desc[0] for desc in cursor.description])
    # Showing the output in the form of table
